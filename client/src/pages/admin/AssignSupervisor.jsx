...
const AssignSupervisor = () => {
  ...
  const Badge = ({ color, children }) => {
    const classes = color || "";
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${classes}`}>{children}</span>
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <div className="overflow-x-auto">
              ...
              <tr>
                {
                  headers.map((h, idx) => {
                    return (
                      <th key={h + idx} className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">
                        {h}
                      </th>
                    );
                  })
                  ...
                  <tr key={row.projectId} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        {
                          row.supervisor !== null ? (
                            <Badge color={"bg-green-100 text-green-800"} children={row.supervisor} />
                          ) : (
                            <Badge color={"bg-red-100 text-red-800"} children={row.status === "rejected" ? "Rejected" : "Not Assigned"} />
                          )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <select className="input-field w-full" value={selectedSupervisor[row.projectId] || ""} disabled={
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <button className="btn-primary text-sm w-40" onClick={() => handleAssign(row.studentId, row.status, row.projectId)}
                        disabled={
          ...
          {
            filtered.length === 0 && (
              <div className="text-center py-8 text-slate-500">No student found matching your criteria</div>
            )
          }
        </div>
