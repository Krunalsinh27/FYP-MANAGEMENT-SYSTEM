...
const AssignedStudents = () => {
  ...
  const [showCompeleteModal, setShowCompeleteModal] = useState(false);
  ...
  const handleMarkComplete = (student) => {
    setSelectedStudent(student);
    setShowCompeleteModal(true);
  };

  const closeModal = () => {
    setShowFeedbackModal(false);
    setShowCompeleteModal(false);
      ...
      <div className="space-y-6">
              ...
              stats.map((item) => {
                return (
                  <div key={item.label} className={`${item.bg} p-4 rounded-lg`}>
                    ...
                    <p className={`text-2xl ${item.valueClass} font-bold`}>{item.value}</p>
                  </div>
                );
              })}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                    <span className="text-blue-600 font-semibold">{student.name?.split(" ").map((n) => n[0]).join("") || "S"}</span>
                  </div>
                  <div>
                ...
                {/* STATUS BADGE */}
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadge(student.project?.status)}`}>{getStatusText(student.project?.status)}</span>
              </div>

              <div className="mb-5">
                <h4 className="font-medium text-slate-700 mb-1">{student.project.title || "No project title"}</h4>
                <p className="text-xs text-slate-500">Last Update:{" "} {new Date(student.project?.updatedAt || new Date()).toLocaleDateString()}</p>
              </div>

              {/* ACTIONS */}
              <div className="flex gap-3">
                <button onClick={() => handleFeedback(student)} className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition">
                  <MessageSquare className="w-4 h-4" />Feedback
                </button>
                <button onClick={() => handleMarkComplete(student)} disabled={student.project?.status === "completed"} className={`flex items-center justify-center gap-2 px-4 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 transition ${student?.project?.status === "complete" ? "opacity-50 cursor-not-allowed" : "hover:bg-green-700"}`}>
          ...
          {sortedStudents.length === 0 && (
            <div className="card text-center py-10 text-slate-600">No assigned students found</div>
          )}
        </div>
                ...
                <div className="bg-slate-50 rounded-lg p-4 mb-6">
                  <div className="space-y-2 text-sm">
                    ...
                    <div>
                      ...
                      <span className="ml-2 text-slate-800">{new Date(selectedStudent.project?.updatedAt || new Date()).toLocaleDateString()}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="mt-5">
                      ...
                      <input type="text" value={feedbackData.title} onChange={(e) => setFeedbackData({ ...feedbackData, title: e.target.value, })} className="w-full px-3 py-2 border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Enter feedback title" />
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-2">Feedback Type</label>

                      <select value={feedbackData.type} onChange={(e) => setFeedbackData({ ...feedbackData, type: e.target.value, })} className="w-full px-3 py-2 border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div>
                      ...
                      <textarea value={feedbackData.message} onChange={(e) => setFeedbackData({ ...feedbackData, message: e.target.value, })} rows={5} className="w-full px-3 py-2 border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none" placeholder="Enter your feedback message..." />
                  ...
                  <div className="flex gap-3 mt-6">
                    ...
                    <button className="btn-primary" onClick={submitFeedback} disabled={!feedbackData.title || !feedbackData.markComplete}>Submit Feedback</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* COMPLETE MODAL */}
        {showCompeleteModal && selectedStudent && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex backdrop-blur-sm items-center justify-center z-50 p-4" onClick={closeModal}>
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-md transform scale-100 transition-all" onClick={(e) => e.stopPropagation()}>
                <div className="p-6">
                  <div className="flex items-center justify-between mb-6">
                  ...
                  <p className="text-slate-600 mb-6">Are you sure to mark this project as completed? This action cannot be undone.</p>

                  <div className="flex gap-3">
                    <button onClick={closeModal} className="btn-danger">Cancel</button>
                    <button onClick={confirmMarkComplete} className="btn-primary">Mark as Completed</button>
                  </div>
                </div>   
              </div>
            </div>
